#!/bin/bash
# [api 서버용] ASG가 새 EC2를 띄울 때 실행되어, SSM에서 설정을 받아 애플리케이션을 가동하는 스크립트
...
#
# ASG/queue/userdata.sh와는 '4. 역할별 설정' 블록만 다르다. 그 외를 고칠 때는 양쪽에 함께 반영한다.
#
...
# SCHEDULER_ENABLED=true — promote·결제·주문 스케줄러는 DB를 읽으므로 api 서버가 전담한다.
cat >> .env <<'ROLE_EOF'
APP_ROLE=api
